...
function paginate_three($reload, $page, $tpages, $adjacents) {
	...
	$prevlabel = "<img src=\"images/icon/arrow_left.png\" width=\"66\" height=\"65\" />";
	$nextlabel = "<img src=\"images/icon/arrow_right.png\" width=\"66\" height=\"65\" />";
...
$sql = "SELECT * FROM product_categories m WHERE 1 = 1 AND (m.status = '1' OR  m.status = '2') ORDER BY m.sequence ASC";
$stmt = $db->Prepare($sql);
$rs = $db->Execute($stmt);
$itemCateList = $rs->GetAssoc();
$itemCateListCount = $rs->maxRecordCount();

$template->itemCateList = $itemCateList;
$template->itemCateListCount = $itemCateListCount;

// status 1 = 'la' , 2 = 'en' , 3 = 'all'
$sql = "SELECT * FROM product_items m WHERE 1 = 1 ".$gettmp['order_txt']." ".$gettmp['order_cat']." AND (m.status = '1' OR  m.status = '2') 
".(($gettmp["keyword"] != null) ? "AND CONCAT_WS(',', m.title_la, m.title_en, m.size_la, m.size_en, m.detail_la, m.detail_en, m.code, m.price) LIKE ".$db->qstr("%".$gettmp["keyword"]."%")." " : "")." ORDER BY m.sequence ASC";
$stmt = $db->Prepare($sql);
...
$gettmp['reload'] = "search.php?load";
$gettmp['KeyParent'] .= ($_GET["parent_id"] != null) ? "&parent_id=".$_GET["parent_id"] : '';
$gettmp['KeySearch'] .= ($_GET["keyword"] != null) ? "&keyword=".$_GET["keyword"] : '';
$gettmp['keyOrder'] .= ($_GET["order_by"] != null) ? "&order_by=".$_GET["order_by"] : '';

$template->Pagereload = $gettmp['reload'].$gettmp['KeyParent'].$gettmp['KeySearch'].$gettmp['keyOrder'];
