...
$sql = "SELECT * FROM product_items m WHERE 1 = 1 ".$gettmp['order_txt']." ".$gettmp['order_cat']." AND m.discount != '' AND (m.status = '1' OR  m.status = '2') ORDER BY m.sequence ASC";
$stmt = $db->Prepare($sql);
...
$template->Pagereload = ($_GET["parent_id"] != null) ? "promotion.php?parent_id=".$_GET["parent_id"] : "promotion.php?load";
