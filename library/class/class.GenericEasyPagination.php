...
Class GenericEasyPagination
{
    ...
    */
	function GenericEasyPagination($page,$recordByPage,$language = "default")
	{
		...
		if ($language == "default"):
			$this->msg_initialPage  = "P๏ฟฝgina Inicial";
			$this->msg_finalPage    = "P๏ฟฝgina Final";
			$this->msg_previousPage = "Anterior";
			$this->msg_nextPage	    = "Pr๏ฟฝxima";
			$this->msg_next10Results = "Pr๏ฟฝximas 10 p๏ฟฝginas";
			$this->msg_previous10Results = "10 P๏ฟฝginas anteriores";
			$this->msg_page		    = "Ir para P๏ฟฝgina";
			$this->msg_of		     = "De";
			$this->msg_to		     = "a";
		else:
    ...
    */
    ...
    function getNavigation_next() {// >>>
        $result = "";
        $previous = $this->currentPage - 1;
        if ($this->getsVars == "") {
            if ($this->currentPage > 1) {
              $result .= '<a href="?page='.($previous).'" class="button" style="background:#ec1992;">NEXT <img src="assets/img/arrow-right.png" /></a>';
            } else { // |<=
              $result .= '<a href="#" class="button button-disabled">NEXT <img src="assets/img/arrow-right.png" /></a>';
            }
        } else {
            if ($this->currentPage > 1) {
              $result .= '<a href="?page='.($previous).'&'.$this->getsVars.'" class="button" style="background:#ec1992;">NEXT <img src="assets/img/arrow-right.png" /></a>';
            } else {
              $result .= '<a href="#" class="button button-disabled">NEXT <img src="assets/img/arrow-right.png" /></a>';
            }
        }    
    ...
    function getNavigation_prev() {// <<<
        $result = "";
        $next = $this->currentPage + 1;
        if ($this->getsVars == "") {
            if (($this->PagesFound > 1) && ($this->currentPage < $this->PagesFound)) {
              $result .= '<a href="?page='.($next).'" class="button" style="background:#ec1992;"><img src="assets/img/arrow-left.png" /> BACK</a>';
            } else {
              $result .= '<a href="#" class="button button-disabled"><img src="assets/img/arrow-left.png" /> BACK</a>';
            }          
        } else {
            if (($this->PagesFound > 1) && ($this->currentPage < $this->PagesFound)) {
              $result .= '<a href="?page='.($next).'&'.$this->getsVars.'" class="button" style="background:#ec1992;"><img src="assets/img/arrow-left.png" /> BACK</a>';
            } else {
              $result .= '<a href="#" class="button button-disabled"><img src="assets/img/arrow-left.png" /> BACK</a>';
            }
        }    
		...
		for ($a = $begin; $a <= $end ; $a++):
			if ($a!=$this->currentPage):
				if ($this->getsVars ==""):
					...
					$result .= " <a href='?page=$a' onMouseOver=\"window.status='".$this->msg_page.": $a';return true\" title='".$this->msg_page.": $a' onMouseOut=\"window.status='';return true\" class=\"page\">$a</a>&nbsp;";
				else:
					...
					$result .= " <a href='?page=$a&".$this->getsVars."' onMouseOver=\"window.status='".$this->msg_page.": $a';return true\" title='".$this->msg_page.": $a' onMouseOut=\"window.status='';return true\" class=\"page\">$a</a>&nbsp;";						
				endif;
			else:
				//$result .= " <strong>[$a]</strong> &nbsp;";
				$result .= " <span class=\"PageActive\">$a</span> &nbsp;";
			endif;
		endfor;	
		return $result;
	}
