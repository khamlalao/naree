...
//    $rs = $db->Execute($stmt, array($gettmp["id"], $gettmp["authen"], $gettmp['date_chk']));
    ...
    $rs = $db->Execute($stmt, array($gettmp["id"], $gettmp["authen"]));
    ...
    if ($itemCount > 0){
      ...
      $gettmp['password'] = createRandomPassword();

      $member->password = $gettmp['password'];

      $member->replace();

      $gettmp['email'] = $member->email;

      $template->email = $gettmp["email"];	  
      ...
      $body = $mail->getFile("tmpl_member_password.html");
