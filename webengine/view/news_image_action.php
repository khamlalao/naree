...
switch ($gettmp["do"]) {
  case "insert" :
      ...
      $sql = "SELECT Count(news_galleries.id) AS amout FROM news_galleries WHERE news_galleries.parent_id = ? ";
      $stmt = $db->Prepare($sql);
    ...
    if ($item->save()) {
       ...
       //       $upload->image_y = 152;
          ...
          //    $upload->image_y = 413;
              ...
              if ($gettmp["file_name"] != null) {
                if (file_exists($upload_path."gallery/".$gettmp["file_name"])) unlink($upload_path."gallery/".$gettmp["file_name"]);
              }
            } else {
        ...
        if ($item->sequence != $gettmp["sequence"]) : 
          $update = true;
          $sql = "SELECT Count(news_galleries.id) AS amount FROM news_galleries WHERE news_galleries.parent_id = ? ";
          $stmt = $db->Prepare($sql);
          ...
          if ($item->sequence != $gettmp["sequence"]) : 
            ...
            if ($item->sequence > $gettmp["sequence"]) : 
              $sql = "UPDATE news_galleries SET news_galleries.sequence = news_galleries.sequence + 1 WHERE news_galleries.sequence >= ? AND news_galleries.sequence < ? AND news_galleries.parent_id = ? ";
              $stmt = $db->Prepare($sql);
      ...
      $render = "news_image_list.php?mn=3&sub=2&parent_id=".$gettmp["parent_id"];
    ...
    case "listupdate" :                            
      ...
      foreach ($_POST["chkbox"] as $row => $gettmp["no"]) :   
        ...
        $stmt = $db->Prepare("SELECT * FROM news_galleries WHERE news_galleries.id = ? ");
        $rs = $db->Execute($stmt, array($gettmp["id"]));
        $sequence = $rs->fields["sequence"];   
        $gettmp["parent_id"] = $rs->fields["parent_id"];   
        
        $stmt = $db->Prepare("SELECT Count(id) AS amount FROM news_galleries WHERE news_galleries.parent_id = ? ");
        $rs = $db->Execute($stmt, array($gettmp["parent_id"]));
        $max = $rs->fields["amount"]; 
        ...
        if ($sequence != $gettmp["to"]) :  
          ...
          if ($sequence > $gettmp["to"]) : 
            $stmt = $db->Prepare("UPDATE news_galleries SET news_galleries.sequence = news_galleries.sequence + 1 WHERE news_galleries.sequence >= ? AND news_galleries.sequence < ? AND news_galleries.parent_id = ? ");
            $rs = $db->Execute($stmt, array($gettmp["to"], $sequence, $gettmp["parent_id"]));
            $stmt = $db->Prepare("UPDATE news_galleries SET news_galleries.sequence = ? WHERE news_galleries.id = ? ");
  ...
  case "listdelete" :
      ...
      foreach ($_POST["chkbox"] as $row => $gettmp["no"]) :   
        ...
        if ($sequence < $max) :                                                                                     
          $stmt = $db->Prepare("UPDATE news_galleries SET news_galleries.sequence = news_galleries.sequence - 1 WHERE news_galleries.sequence > ?  AND news_galleries.parent_id = ? ");
        ...
        $stmt = $db->Prepare("DELETE FROM news_galleries WHERE news_galleries.id = ? ");
        $rs = $db->Execute($stmt, array($gettmp["id"]));  
      endforeach;
  ...
  case "delete" : 
    ...
    case "status" :              
      ...
      $stmt = $db->Prepare("UPDATE news_galleries SET news_galleries.status = ? WHERE news_galleries.id = ? ");
