...
$upload_path = "../../img_product_gallery/";
...
switch ($gettmp["do"]) {
  case "insert" :
      ...
      $sql = "SELECT Count(product_galleries.id) AS amout FROM product_galleries WHERE product_galleries.parent_id = ? ";
      $stmt = $db->Prepare($sql);
      $rs = $db->Execute($stmt, array($gettmp["parent_id"]));
      $gettmp["sequence"] = $rs->fields["amout"] + 1; 
  
    class product_gallery extends ADOdb_Active_Record{}
    $item = new product_gallery();
    ...
    if ($item->save()) {
      ...
      if ($_FILES["file_name"] != null) {
       ...
       //       $upload->image_y = 152;
        ...
        if ($item->sequence != $gettmp["sequence"]) : 
          $update = true;
          $sql = "SELECT Count(product_galleries.id) AS amount FROM product_galleries WHERE product_galleries.parent_id = ? ";
          $stmt = $db->Prepare($sql);
          ...
          if ($item->sequence != $gettmp["sequence"]) : 
            ...
            if ($item->sequence > $gettmp["sequence"]) : 
              $sql = "UPDATE product_galleries SET product_galleries.sequence = product_galleries.sequence + 1 WHERE product_galleries.sequence >= ? AND product_galleries.sequence < ? AND product_galleries.parent_id = ? ";
              $stmt = $db->Prepare($sql);
      ...
      $render = "product_image_list.php?mn=6&sub=1&parent_id=".$gettmp["parent_id"];
    ...
    case "listupdate" :                            
      ...
      foreach ($_POST["chkbox"] as $row => $gettmp["no"]) :   
        ...
        $stmt = $db->Prepare("SELECT * FROM product_galleries WHERE product_galleries.id = ? ");
        $rs = $db->Execute($stmt, array($gettmp["id"]));
        $sequence = $rs->fields["sequence"];   
        $gettmp["parent_id"] = $rs->fields["parent_id"];   
        
        $stmt = $db->Prepare("SELECT Count(id) AS amount FROM product_galleries WHERE product_galleries.parent_id = ? ");
        $rs = $db->Execute($stmt, array($gettmp["parent_id"]));
        $max = $rs->fields["amount"]; 
        ...
        if ($sequence != $gettmp["to"]) :  
          ...
          if ($sequence > $gettmp["to"]) : 
            $stmt = $db->Prepare("UPDATE product_galleries SET product_galleries.sequence = product_galleries.sequence + 1 WHERE product_galleries.sequence >= ? AND product_galleries.sequence < ? AND product_galleries.parent_id = ? ");
            $rs = $db->Execute($stmt, array($gettmp["to"], $sequence, $gettmp["parent_id"]));
            $stmt = $db->Prepare("UPDATE product_galleries SET product_galleries.sequence = ? WHERE product_galleries.id = ? ");
  ...
  case "listdelete" :
      ...
      foreach ($_POST["chkbox"] as $row => $gettmp["no"]) :   
        ...
        if ($sequence < $max) :                                                                                     
          $stmt = $db->Prepare("UPDATE product_galleries SET product_galleries.sequence = product_galleries.sequence - 1 WHERE product_galleries.sequence > ?  AND product_galleries.parent_id = ? ");
        ...
        $stmt = $db->Prepare("DELETE FROM product_galleries WHERE product_galleries.id = ? ");
        $rs = $db->Execute($stmt, array($gettmp["id"]));  
      endforeach;
  ...
  case "delete" : 
    ...
    case "status" :              
      ...
      $stmt = $db->Prepare("UPDATE product_galleries SET product_galleries.status = ? WHERE product_galleries.id = ? ");
